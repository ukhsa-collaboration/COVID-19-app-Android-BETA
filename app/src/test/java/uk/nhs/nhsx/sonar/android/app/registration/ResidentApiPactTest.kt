...
class ResidentApiPactTest {
    @Rule
    @JvmField
    val provider = PactProviderRule("resident_api_provider", this)
    ...
    fun pact(builder: PactDslWithProvider): RequestResponsePact {
        return builder
            ...
            .method("POST")
            .body(PactDslJsonBody().stringMatcher("pushToken", ".*", "a-valid-token"))
            // response
            .willRespondWith()
    ...
    fun `verifies pact with resident api provider`() {
        ...
        val request = residentApi.register("a-valid-token")
